...
/**
 * Componente ChecklistForm
 *
 * Descrição:
 * Este arquivo define o formulário utilizado nos processos de check-in e check-out.
 * Ele renderiza uma lista de todos os itens do inventário da propriedade, permitindo
 * que o usuário verifique e reporte o estado de conservação de cada um.
 *
 * Funcionalidades:
 * - Gerencia o estado do checklist, inicializando-o com os dados do inventário.
 * - Permite a alteração do estado de conservação de cada item.
 * - Exibe um campo de observação obrigatório caso um item seja marcado como
...
/**
 * Renderiza uma linha individual para cada item no formulário do checklist.
 */
const ChecklistItem = React.memo(({ item, itemState, onItemStateChange }) => {
...
/**
 * Componente de formulário principal para o checklist.
 */
const ChecklistForm = ({ inventoryItems, onSubmit, checklistType, isLoading }) => {
   ...
   */
  const handleItemStateChange = useCallback((itemId, field, value) => {
    setChecklist(prev => ({
      ...prev,
  ...
  const handleSubmit = useCallback((e) => {
    ...
    onSubmit(payload);
  }, [isLoading, checklist, generalObservations, onSubmit]);
    ...
    <div className="bg-white rounded-2xl shadow-md p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end pt-4">
          {/* Adiciona classes para largura mínima (w-100), flexbox e centralização. */}
          <button
            type="submit"
            disabled={isLoading}
            className="px-6 py-3 bg-black text-white font-bold rounded-lg hover:bg-gray-800 transition disabled:bg-gray-400 flex justify-center items-center w-100"
          >
